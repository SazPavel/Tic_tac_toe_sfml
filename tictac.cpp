...
int main(){
	...
	sf::Texture kresty;
	sf::Sprite krest[5];
	if (!kresty.loadFromFile("resources/circles.png", sf::IntRect(0, 0, TILESIZE, TILESIZE)))
    	return -1;
	for(int i = 0; i< 5; i++)	
		krest[i].setTexture(kresty);
	
	int KRUG[5];
	sf::Texture kruglton;
	sf::Sprite krug[5];
	if (!kruglton.loadFromFile("resources/circles.png", sf::IntRect(TILESIZE, 0, TILESIZE, 2 * TILESIZE)))
    	return -1;	
	for(int i = 0; i< 5; i++)	
		krug[i].setTexture(kruglton);
	...
	if (!font.loadFromFile("resources/sansation.ttf"))
        return -1;	
    // создать tilemap из определения уровня
    TileMap map;
    int level[9] = {0, 1, 0, 1, 0, 1, 0, 1, 0};
    if (!map.load("resources/tileset1.png", sf::Vector2u(TILESIZE, TILESIZE), level, 3, 3))
        return -1;
	int diff;
    ...
    while (window.isOpen()){
        ...
        while (window.pollEvent(event)){
            ...
            if (event.type == sf::Event::MouseButtonPressed){
           	 	if(event.mouseButton.x >= 408 && event.mouseButton.y <= 428 && event.mouseButton.y >= 384) {  
           	 	game = 1;
           	 	j = 0;
           	 	i = 0;
           	 	k = 0;
			...
			if(game == 1 && wh == 0){
           		if (event.type == sf::Event::MouseButtonPressed){           	 			
           	 		if(event.mouseButton.x <= 190 && event.mouseButton.y <= 190) {
           	 			if(i == 0){
							KREST[j] = event.mouseButton.y/TILESIZE * 3 + event.mouseButton.x/TILESIZE;
							if(arr[KREST[j]] == 0){							
								location(krest[j], KREST[j]);
								arr[KREST[j]] = 1;
								i = 1;
           	 					j += 1;
							}
						...
						if(k < 4 && i == 1){
							KRUG[k] = AI(arr, 2, diff);
							location(krug[k], KRUG[k]);
							arr[KRUG[k]] = 2;
							k += 1;
							i = 0;
						}		
